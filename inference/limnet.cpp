...
void limnet_thread_func() {
    ...
    while (!needs_to_terminate) {
        message_t message;
        if (!message_queue.wait_dequeue_timed(message, std::chrono::milliseconds(1000))) {
            std::cout << "WAITING" << std::endl;
            continue;
        }
        switch (message.index()) {
            case 0: {
                std::cout << "BEGIN CASE 0" << std::endl;
                ...
                auto pkt_features = torch::from_blob(&packet->features, {(int64_t)(NUM_PROTOS+5)}, c10::TensorOptions().dtype(c10::kFloat));
                auto new_memories = packet_model.forward({src_mem->memory, dst_mem->memory, pkt_features}).toTuple();
                auto new_dst_mem = new_memories->elements()[0].toTensor();
                auto new_src_mem = new_memories->elements()[1].toTensor();
                auto packet_class = packet_cls_model.forward({new_src_mem, new_dst_mem, pkt_features}).toTensor().item<uint8_t>();
                auto src_class = device_cls_model.forward({new_src_mem}).toTensor().item<uint8_t>();
                auto dst_class = device_cls_model.forward({new_dst_mem}).toTensor().item<uint8_t>();
                {
                    std::lock_guard lock(src_mem->mutex);
                    src_mem->memory = new_src_mem;
                    src_mem->flags = src_class;
                    src_mem->score += 1;
                }
                {
                    std::lock_guard lock(dst_mem->mutex);
                    dst_mem->memory = new_dst_mem;
            ...
            case 1: {
                ...
                for (size_t i = 0; i < num_entries; i++) {
                    ...
                    if (memories.contains(ip)) {
                        ...
                        mem->flags = device_cls_model.forward({mem->memory}).toTensor().item<uint8_t>();
                        mem->score += 1;
                ...
                if (cutoff > 0) {
                    ...
                    for (size_t i = 0; i < cutoff; i++) {
                        ...
                        auto new_class = device_cls_model.forward({new_mems[index]}).toTensor().item<uint8_t>();
...
extern "C" void on_memories_received(ip_t* ips, void* memories, size_t num_entries) {
    ...
    auto has_forgeries = forgery_model.forward({decompressed_memories}).toTensor().item<bool>();
    if (!has_forgeries)
